...
int main()
{
    ...
    // second, configure the light's VAO (VBO stays the same; the vertices_x are the same for the light object which is also a 3D cube)
    unsigned int lightCubeVAO_x;
    glGenVertexArrays(1, &lightCubeVAO_x);
    glBindVertexArray(lightCubeVAO_x);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        lightingShader.setFloat("pointLights[0].one", 0.09f);
        lightingShader.setFloat("pointLights[0].two", 0.032f);
        // point light 2
        ...
        lightingShader.setFloat("pointLights[1].one", 0.09f);
        lightingShader.setFloat("pointLights[1].two", 0.032f);
        // point light 3
        ...
        lightingShader.setFloat("pointLights[2].one", 0.09f);
        lightingShader.setFloat("pointLights[2].two", 0.032f);
        // point light 4
        ...
        lightingShader.setFloat("pointLights[3].one", 0.09f);
        ...
        //画出来草丛或者窗户。但是因为先进行了深度测试，所以会导致窗户的遮挡行为。
        //需要按照从后到前的顺序来绘制窗户。
        grassShader.use();
        ...
        int length = sizeof(vegetation) / sizeof(vegetation[0]);
        for (int i = 0; i < length; i++) {
            float d = glm::length(camera.Position - vegetation[i]);
            sorted[d] = vegetation[i];
        }

        //逆序输出即可
        std::map<float, glm::vec3>::reverse_iterator iter;
    ...
    glDeleteVertexArrays(1, &lightCubeVAO_x);
